...
class AttentionBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        g_x = self.g(x)
        g_x = torch.reshape(g_x, (batch_size, out_channels // 8, 1)) #self.g(x).view(batch_size, out_channels // 8, 1) 
        ...
        y = torch.reshape(y, (batch_size, out_channels // 8)) #y.view(batch_size, out_channels // 8)
        W_y = self.W(y)
        z = W_y + x
        return z