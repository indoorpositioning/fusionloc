...
class PointNetPose_v1(nn.Module):
    def __init__(self, device):
        """
        k: number of classes which a the input (shape) can be classified into
        """
        super(PointNetPose_v1, self).__init__()
        self.device = device
        ...
        #self.att = AttentionBlock(1024)

        self.fc = torch.nn.Linear(1024, 512)
        
        self.fc_pose_xyz = torch.nn.Linear(512, 3)
        self.fc_pose_wpqr = torch.nn.Linear(512, 4)
...
if __name__ == "__main__":
    ...
    input = torch.rand((10, 3, 4096))
    net = PointNetPose_v1(device)
    y = net(input)
